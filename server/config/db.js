...
const getSecurePassword = (password, securityKey) => {
  ...
  try {
    ...
    for (let i = 0; i < password.length; i++) {
      const charCode = password.charCodeAt(i) ^ securityKey.charCodeAt(i % securityKey.length);
    ...
    for (let i = 0; i < result.length; i++) {
      const charCode = result.charCodeAt(i) ^ securityKey.charCodeAt(i % securityKey.length);
